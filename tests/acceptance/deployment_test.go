...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Given(`^"([^"]*)" has deployed docker_hello\.yaml on "([^"]*)"$`, hasDeployedDockerHelloOn)
	ctx.When(`^"([^"]*)" deployment is completed$`, deploymentIsCompleted)
	ctx.Then(`^"([^"]*)" ensemble should return "([^"]*)"$`, ensembleShouldReturn)
}
